...
<section class="content-header no-print">
    <h1><a href="/loans/{{$loan->id}}"> <span class="fa fa-arrow-left"></span> @lang( 'messages.back' )</a></h1>
</section>
<!-- Main content -->
<section class="content no-print">
    <h3>{{$customer->supplier_business_name}}{{$customer->name}} &nbsp; <small>{{date("d  M, Y",strtotime($payment_schedule->sheduled_date))}}</small></h3>
    <!-- Nueva tabla -->
                            ...
                            <tr>
                                ...
                                <td >
                                    @if($delay->status == "late")
                                        ...
                                        <a href="{{route('add.pay.delay',$delay->id)}}" data-type="pay" class="btn btn-xs btn-success add_payment_modal">
                                            <i class="fas fa-money-bill-alt"></i> Agregar Pago
                                        </a> &nbsp;

                                         <a href="{{route('add.pay.delay',$delay->id)}}" data-type="partial" class="btn btn-xs btn-info add_payment_modal">
                                            <i class="fas fa-money-bill-alt"></i> Agregar y Condonar Parcial
                                        </a> &nbsp;

                                        <button type="button" id="button-condonar" class="btn btn-xs btn-warning" data-id="{{$delay->id}}">
                                            <i class="fas fa-money-bill-alt"></i> Condonar
                                        </button> &nbsp;

                                        <button data-href="{{action('App\Http\Controllers\DelayController@destroy', [$delay->id])}}" class="btn btn-xs btn-danger delete_delay_button">
