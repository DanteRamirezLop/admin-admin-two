    ...
    {!! Form::open(['action' => '\Modules\Repair\Http\Controllers\JobSheetController@store', 'id' => 'job_sheet_form', 'method' => 'post', 'files' => true]) !!}
        ...
        <div class="box box-solid">
            <div class="box-body">
                ...
                <div class="col-lg-6">
                    {!! Form::label('custom_field_5', $custom_field_4_label . ':') !!} <br>
                    <canvas id="signature-customer" width="440" height="200" style="border:1px solid #000;"></canvas><br>
                    <button type="button" class="btn btn-primary text-white" id="clearCustomer">Borrar Firma</button>
                    <input type="hidden" name="custom_field_4" id="firmaInputCustomer">
                </div>

                <div class="col-lg-6">
                    {!! Form::label('custom_field_5', $custom_field_5_label . ':') !!} <br>
                    <canvas id="signature-technique" width="440" height="200" style="border:1px solid #000;"></canvas><br>
                    <button type="button" class="btn  btn-primary text-white" id="clearTechnique">Borrar Firma</button>
                    <input type="hidden" name="custom_field_5" id="firmaInputTechnique">
                </div>
...
@section('javascript')
    ...
    <script type="text/javascript">
        $(document).ready( function() {
            ...
            const canvasCustomer = document.getElementById('signature-customer');
            const signaturePadCustomer = new SignaturePad(canvasCustomer);
            const canvasTechnique = document.getElementById('signature-technique');
            const signaturePadTechnique = new SignaturePad(canvasTechnique);
            ...
            $('.submit_button').click( function(){
                //***Firma***
                    ...
                    if (signaturePadCustomer.isEmpty()) {
                        $('#firmaInputCustomer').val('');
                        $('#clearCustomer').after('<strong class="error"> This field is required. </strong>');
                        return false;
                    }else{
                        const dataURLCustomer = signaturePadCustomer.toDataURL();
                        $('#firmaInputCustomer').val(dataURLCustomer);
                    }

                    if(signaturePadTechnique.isEmpty()){
                        $('#firmaInputTechnique').val('');
                        $('#clearTechnique').after('<strong class="error"> This field is required. </strong>');
                        return false;
                    }else{
                        const  dataURLTechnique = signaturePadTechnique.toDataURL();
                        $('#firmaInputTechnique').val(dataURLTechnique);
                    }
