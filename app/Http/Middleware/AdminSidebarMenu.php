...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            if (auth()->user()->can('user.view') || auth()->user()->can('user.create') || auth()->user()->can('roles.view')) {
                $menu->dropdown(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('goal.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\GoalController::class, 'index']),
                                __('user.goals'),
                                ['icon' => 'fa fas fa-briefcase', 'active' => request()->segment(1) == 'goals']
                            );
                        }
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (! empty($common_settings['enable_purchase_order']) && (auth()->user()->can('purchase_order.view_all') || auth()->user()->can('purchase_order.view_own'))) {
                            ...
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'purchase-order' && request()->segment(2) == 'create']
                            );
                        }
           ...
           if (auth()->user()->can('customer.view_own') &&  auth()->user()->can('category.view')) {
                $menu->dropdown(
                    'Cotizador',
                     function ($sub) {

                        $sub->url(
                            action([\App\Http\Controllers\CotizarController::class, 'index']),
                            'Lista de cotizaciones',
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'cotizar'  && request()->segment(2) == null]
                        );

                         $sub->url(
                             action([\App\Http\Controllers\CotizarController::class, 'create']),
                             'Crear cotización',
                           ...
                           if (auth()->user()->can('business_settings.access')) {
                            $sub->url(
                                ...
                                'Configuraciones',
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'cotizacion-confi' && request()->segment(2) == null]
                            );
                         }
                         
                         if (auth()->user()->can('business_settings.access')) {
                            $sub->url(
                                action([\App\Http\Controllers\CotizarController::class, 'report']),
                                'Reportes',
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'reporte-cotizacion' && request()->segment(2) == null]
                            );
                        }else{ 
                            $sub->url(
                                action([\App\Http\Controllers\CotizarController::class, 'reportWaiter']),
                                'Reportes',
                                ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'reporte-waiter' && request()->segment(2) == null]
                            );   
                        }
            ...
            //prestamos
            if ($is_admin || auth()->user()->hasAnyPermission(['unit.view', ])) {
                $menu->dropdown(
                    __('loan.loan'),
                     function ($sub) {
                        $sub->url(
                            action([\App\Http\Controllers\LoanController::class, 'index']),
                            __('loan.lending_list'),
                            ['icon' => 'fa fas fa-list', 'active' => request()->segment(1) == 'loans'  && request()->segment(2) == null]
                        );
                        if (auth()->user()->can('user.view') || auth()->user()->can('user.create') || auth()->user()->can('roles.view')){ 
                         $sub->url(
