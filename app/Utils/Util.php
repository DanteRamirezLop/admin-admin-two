...
class Util
{
    ...
    public function replaceTags($business_id, $data, $transaction, $contact = null)
    {
        ...
        foreach ($data as $key => $value) {
            ...
            $transaction_sell_line = TransactionSellLine::where('transaction_id', $transaction->id)->first();
            $media = Media::where('model_id',$transaction_sell_line->product->id)->first();
            
            $ficha_tecnica = env('APP_URL') .'uploads/media/' . $media->file_name;
            $data[$key] = str_replace('{ficha_tecnica}', $ficha_tecnica, $data[$key]);
            
             $cotizacion = env('APP_URL') .'uploads/documents/' . $transaction->document;
            $data[$key] = str_replace('{cotizacion}', $cotizacion, $data[$key]);
        }
