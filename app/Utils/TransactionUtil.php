...
class TransactionUtil extends Util
{
    ...
    private function simulateFrenchScheduleTotals( float $startingBalance, float $monthlyRate, int $n, float $installment ): array 
    {
     ...
     public function getTotalInteresPaid($loan_id)
    {
        $schedule_version_current_id = NULL;
        $schedule_version_current = ScheduleVersion::where('loan_id',$loan_id)->where('status','active')->first();
        if ($schedule_version_current) {
            $schedule_version_current_id = $schedule_version_current->id;
        }
        $paymentSchedule = PaymentSchedule::where('loan_id',$loan_id)->where('status','pending')->where('schedule_version_id',$schedule_version_current_id)->get();
        $total_paid = 0;
        foreach ($paymentSchedule as $payment) {
            $total_paid += $payment->interests;
        }
        return $total_paid;
    }

    public function getOtherExpensesPaid($loan_id)
    {
        $schedule_version_current_id = NULL;
        $schedule_version_current = ScheduleVersion::where('loan_id',$loan_id)->where('status','active')->first();
        if ($schedule_version_current) {
        ...
        $paymentSchedule = PaymentSchedule::where('loan_id',$loan_id)->where('status','pending')->get();
        $total_paid = 0;
        foreach ($paymentSchedule as $payment) {
            $total_paid += $payment->gps_quota + $payment->sure_quota + $payment->admin_fee_quota + $payment->initial;
        }
        return $total_paid;
    }
    ...
    public function createExpense($request, $business_id, $user_id, $format_data = true)
    {
        ...
        $transaction_data['staff_note'] = ! empty($request->input('expense_details')) ? json_encode($request->input('expense_details')) : null;
